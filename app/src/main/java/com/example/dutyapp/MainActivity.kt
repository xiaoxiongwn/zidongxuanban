...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnBaseDate.setOnClickListener {
            pick(baseDate)
        }

        btnMode.setOnClickListener {
            val options = arrayOf("上一休一", "上一休二", "上一休三")

            AlertDialog.Builder(this)
                .setTitle("选择排班方式")
                .setItems(options) { _, which ->
                    cycle = which + 2
                    btnMode.text = options[which]
                    calc()
                }
                .show()
        }
    ...
    private fun calc() {
        val diff = (
            (selectedDate.timeInMillis - baseDate.timeInMillis)
                    / (1000 * 60 * 60 * 24)
        ).toInt()

        txtResult.text =
            if (diff % cycle == 0) "上班" else "休息"
    }
}
